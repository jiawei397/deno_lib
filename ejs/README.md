...
```ts
import { render } from "https://deno.land/x/deno_ejs/mod.ts";
...
```ts
import { renderFile } from "https://deno.land/x/deno_ejs/mod.ts";
renderFile(
  "./template.ejs",
  {
    title: "from test2",
...
- `includer` Custom function to handle EJS includes, receives
  `(originalPath, parsedPath)` parameters, where `originalPath` is the path in
